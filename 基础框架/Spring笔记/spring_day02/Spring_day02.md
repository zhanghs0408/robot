...
resources下创建一个jdbc.properties文件,并添加对应的属性键值对,为什么要在键的前面加个jdbc呢？

因为不加jdbc可能会导致出现错误，像username是先加载系统环境变量里的username的，就可能导致出现一些错误
...
在applicationContext.xml中开`context`命名空间，给xmlns取context名 
...
<beans xmlns="http://www.springframework.org/schema/beans"
    ...
    <context:property-placeholder location="classpath:jdbc.properties"/>//classpath:加上比较规范，这里也可以识别*通配符如*.properties
...
从图中可以看出，容器类也是从无到有根据需要一层层叠加上来的，大家重点理解下这种设计思想。![image-20230910145842943](assets/image-20230910145842943.png)
...
- ApplicationContext接口常用初始化类
  ...
  - **AnnotationConfigApplicationContext(常用)**//加载注解
...
* **@Component注解如果不起名称，会有一个默认值就是`当前类名首字母小写`，所以也可以按照名称获取，如**（重要）
...
在配置类上添加`@Configuration`注解，将其标识为一个配置类,替换`applicationContext.xml`（不是必须的，有些情况可以省，建议加）
...
在配置类上添加包扫描注解`@ComponentScan`替换`<context:component-scan base-package=""/>`（注解怎么包扫描的）
...
* 先给两个Dao类分别起个名称
  ...
  * @Autowired默认按照类型自动装配，如果IOC容器中同类的Bean找到多个，**就按照变量名bookDao和Bean  bookDao的名称匹配**。因为变量名叫`bookDao`而容器中也有一个`bookDao`，所以可以成功注入。

  * 分析下面这种情况是否能完成注入呢?（都没有的情况）
...
==**注意:@Qualifier不能独立使用，必须和@Autowired一起使用==**（成员变量时一起使用，方法的形参就可以单独使用）
...
* 如果读取的properties配置文件有多个，可以使用`@PropertySource`的属性来指定多个（value是一个String类型的数组）
...
**@Bean注解的作用是将方法的返回值制作为Spring管理的一个bean对象（默认值是方法名）也可以赋值**
...
public class App {
    public static void main(String[] args) {
        AnnotationConfigApplicationContext ctx = new AnnotationConfigApplicationContext(SpringConfig.class);
        DataSource dataSource = ctx.getBean("dateSource",DataSource.class);
        System.out.println(dataSource);
    }
}
```
...
![image-20230222145555930](C:\Users\hp\AppData\Roaming\Typora\typora-user-images\image-20230222145555930.png)
...
>
> 3.使用@PropertySource加载jdbc.properties配置文件（类上@PropertySource("classpath:jdbc.properties")）
>
> 4.修改@Value注解属性的值，将其修改为`${key}`，key就是键值对中的键的值

具体的实现就交由大家自行实现下。

![image-20230222132115593](C:\Users\hp\AppData\Roaming\Typora\typora-user-images\image-20230222132115593.png)
...
```java
@Bean
public DataSource dataSource(BookDao bookDao){//@Autowired省略了，跟那边用的基本原理一样
    System.out.println(bookDao);
    DruidDataSource ds = new DruidDataSource();
    ds.setDriverClassName(driver);
    ds.setUrl(url);
...
![image-20230222151010791](C:\Users\hp\AppData\Roaming\Typora\typora-user-images\image-20230222151010791.png)
...
#### 步骤5:创建Mybatis配置类并配置SqlSessionFactory（重点）
...
![image-20230223113537791](C:\Users\hp\AppData\Roaming\Typora\typora-user-images\image-20230223113537791.png)
