...
**//Dao层方法返回值类型有void，long，bollean，Integer四种类型**
...
public class Result{
	...
	private Integer code;  // 自定义的一个操作状态码  20040  20041
	private String msg;    // 提示消息(多用于失败)
}
```
...
**说明:**此注解自带@ResponseBody注解与@Component注解，具备对应的功能，复合注解
...
- 其他异常（Exception）
  ...
  - 记录日志
  - ![image-20230228091530099](C:\Users\hp\AppData\Roaming\Typora\typora-user-images\image-20230228091530099.png)
...
public class SystemException extends RuntimeException{//RuntimeException比继承Exception要好
    private Integer code;
    ...
    //常用，多用于将用户的非法操作转换成异常对象，但是不包含原始异常
    public SystemException(Integer code, String message) {
        super(message);
        this.code = code;
    }
    //常用，多用于将用户的非法操作转换成异常对象，但是包含原始异常
...
2. 因为添加了静态资源，SpringMVC会拦截，所以需要在SpringMVCConfig的配置类中将静态资源进行放行。
...
>
> 4.访问的路径为`http://localhost/books`//80端口是这个
>
> 5.返回数据
...
public class SpringMvcConfig implements WebMvcConfigurer {
    ...
    public void addInterceptors(InterceptorRegistry registry) {
        //配置多拦截器
        registry.addInterceptor(projectInterceptor).addPathPatterns("/books","/books/*");//*和**的区别*代表匹配到一层路径，**匹配任意路径
...
public Interceptor interceptor(){
    ...
    // 当传递的页码超过总页数时，默认返回最后一页的数据
    properties.setProperty("reasonable","true");
    interceptor.setProperties(properties);
    return interceptor;
}

// 配置一个用于产生SqlSessionFactory对象的Bean
@Bean
public SqlSessionFactoryBean ssfb(DataSource dataSource,Interceptor interceptor){
...
```java
public PageInfo getPage(Integer currentPage, Integer pageSize) {
    // 通过PageHelper的startPage方法设置分页参数
...
public void testGetPage() throws Exception{
    PageInfo pageInfo = bookService.getPage(1, 3);
